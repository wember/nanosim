...
def Su_stable(N: int, N0: int, Nx: int, N0_exp: int) -> float:
    ...
    return (
        logg(N + 1) + log_2_term - (logg(N - N0 - Nx + 1) + logg(N0 + 1) + logg(Nx + 1))
    )
...
def create_argument_parser(sim_type: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--n", type=int, default=1000000, help="Lattice size (default: 1000000)"
    )
    parser.add_argument(
        "--s", type=int, default=5000, help="Number of sweeps per phase (default: 5000)"
    )
    parser.add_argument(
        "--r",
        type=int,
    ...
    parser.add_argument(
        "--m", type=int, default=5, help="Number of independent runs (default: 5)"
    )
    parser.add_argument(
        "--cores",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Validation mode: off (fastest), periodic (every 100 sweeps), "
...
def build_simulation_parameters(
    r: int, m: int, n: int, s: int, validate_mode: str, project_root: str, use_jit: bool
):
    ...
    for M in range(m):
        for R in range(r):
            sim_params.append(
                (R, M, n, s, validate_mode, project_root, use_jit, sim_num, None)
            )
            sim_num += 1
    return sim_params
...
def print_final_results(results, start_time, end_time, sim_type: str):
    ...
    for result in results:
        if abs(result["E_total"] - result["E_initial"]) > 1e-10:
            ...
            logging.warning(
                f"Energy conservation error: R={result['R']}, M={result['M']}"
            )

    if energy_errors:
        print(
            f"\nWARNING: {len(energy_errors)} simulations had "
...
def calculate_progress_description(
    ...
    if sim_times:
        ...
        return (
            f"Overall Progress (Running {active_count} | Ex: R={first_sim['R']}, "
            f"M={first_sim['M']}, {first_sim['phase']} {first_sim['progress']}/{s} "
            f"{pct:.0f}% | ETA: ~{format_time(eta_minutes)} remaining)"
        )
        ...
        return (
            f"Overall Progress (Running {active_count} | Ex: R={first_sim['R']}, "
            f"M={first_sim['M']}, {first_sim['phase']} {first_sim['progress']}/{s} "
            f"{pct:.0f}% | Est: ~{format_time(total_eta_minutes)} remaining)"
        )
    else:
        # Show running status with example (before 1% threshold)
        return (
            f"Overall Progress (Running {active_count} | "
            f"Ex: R={first_sim['R']}, M={first_sim['M']}, "
            f"{first_sim['phase']} {first_sim['progress']}/{s} {pct:.0f}%)"
        )


def handle_progress_message(
    msg, active_sims, s, pbar, last_refresh, sim_times, total_sims, num_cores, completed
):
    """
    ...
    if now - last_refresh >= 1.0:
        desc = calculate_progress_description(
            active_sims, sim_times, s, total_sims, num_cores, completed
        )
            ...
            try:
                ...
                if msg["type"] == "start":
                    ...
                    if msg["sim_num"] in active_sims:
                        elapsed = (
                            time.time() - active_sims[msg["sim_num"]]["start_time"]
                        )
...
class SimulationBase:
    ...
    def initialize_lattice(N):
        ...
        lattice = np.concatenate(
            (np.ones(N // 2, dtype=np.int8), (-1) * np.ones(N // 2, dtype=np.int8))
        )
    ...
    def spin_flip(self, a, i):
        ...
        Returns:
            bool: True if spin was flipped (bonds may have changed),
                False otherwise
        """
        s = self.lattice[a]
        d = self.E_demon[i]
    ...
    def update_bonds_incremental(self, a):
        ...
        if self.bonds[a] != 0:
            ...
            new_bond = np.int8(
                -1 if self.lattice[a] == self.lattice[self.right_neighbor[a]] else 1
            )
