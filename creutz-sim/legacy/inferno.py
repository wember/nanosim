...
class Inferno:
    ...
    def spin_flip(self, a, i):
        # print(self.lattice, "lattice: ", self.E_lattice,  "demon: ", self.E_demon,
        #  self.d_energy, "total: ", self.E_lattice+sum(self.E_demon))
        ...
        #     uk = (f(self.N)*2**(self.bond_count[1]+1))/(f(self.N-self.bond_count[1]-
        #       self.bond_count[2])*f(self.bond_count[1])*f(self.bond_count[2]))
        #     SUtest = Su0(self.N, self.bond_count[1], self.bond_count[2])
        # else:
        #     uk = (f(self.N)*2**(self.bond_count[1]))/(f(self.N-self.bond_count[1]-
        #       self.bond_count[2])*f(self.bond_count[1])*f(self.bond_count[2]))
        #     SUtest = Su(self.N, self.bond_count[1], self.bond_count[2])
        # sk = f(self.d_energy+self.N-1)/(f(self.d_energy)*f(self.N-1))
        # # print(f" {self.bond_count[1]}  |  {self.bond_count[2]} | {self.E_lattice}
        #       |ln({uk})|  {self.d_energy}  |ln({sk})|ln({sk*uk})")
        #
        ...
        # Calculate the energy of the configuration based on
        # nearest neighbors
        nb = self.lattice[(a + 1) % self.N] * abs(
            self.bonds[(a) % self.N]
    ...
    def count_bonds(self):
        """
        Updates the bond-count array of number of aligned (-1), broken (0), and
        misaligned (-1) bonds
        """
