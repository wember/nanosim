...
These functions are extracted from the SimulationBase class methods and optimized for speed.
...
def spin_flip_jit(lattice, bonds, E_demon, E_demon_sum, E_lattice, 
    ...
    nb = (lattice[right_neighbor[a]] * abs(bonds[a]) +
          lattice[left_neighbor[a]] * abs(bonds[left_neighbor[a]]))
...
@njit(cache=True)
def update_bonds_incremental_jit(lattice, bonds, bond_count, 
                                  right_neighbor, left_neighbor, a):
    """
...
@njit(cache=True)
def bond_change_jit(lattice, bonds, bond_count, E_demon, E_demon_sum, E_lattice,
                   right_neighbor, left_neighbor, a, i):
    """
...
def demon_move_jit(lattice, bonds, bond_count, E_demon, E_demon_sum, E_lattice,
    ...
    for j in range(N):
        ...
        bonds_changed, E_demon_sum, E_lattice = spin_flip_jit(
            lattice, bonds, E_demon, E_demon_sum, E_lattice,
            right_neighbor, left_neighbor, a, i
        )
        ...
        E_demon_sum, E_lattice, bond_count = bond_change_jit(
            lattice, bonds, bond_count, E_demon, E_demon_sum, E_lattice,
            right_neighbor, left_neighbor, a, i
        )
...
@njit(cache=True)
def demon_move_irr_jit(lattice, bonds, bond_count, E_demon, E_demon_sum, E_lattice,
                       right_neighbor, left_neighbor, order, N, R, seed):
    """
