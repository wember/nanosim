...
class TestLongRunStability:
    ...
    def test_multiple_forward_reverse_cycles(self):
        ...
        np.testing.assert_array_equal(
            x.bonds, initial_bonds, err_msg="Bonds not restored after multiple cycles"
        )
...
class TestValidationBranches:
    ...
    def test_inferno_frequent_validation_in_demon_reverse(self):
        ...
        # Run exactly N moves to trigger validation (check_interval = N for
        # frequent mode)
    ...
    def test_irr_inferno_frequent_validation_in_demon_move(self):
        """Test that frequent validation triggers validation checks in
        irrInferno demon_move."""
        sim = irrInferno(N=100, R=5, validate_mode="frequent")
        initial_total = sim.E_lattice + sim.E_demon_sum

        # Run exactly N moves to trigger validation (check_interval = N
        # for frequent mode)
        for _ in range(sim.N):
            sim.demon_move()
    ...
    def test_irr_inferno_frequent_validation_in_demon_reverse(self):
        """Test that frequent validation triggers validation checks
        in irrInferno demon_reverse."""
        sim = irrInferno(N=100, R=5, validate_mode="frequent")
        initial_total = sim.E_lattice + sim.E_demon_sum
...
class TestValidationMethodAccuracy:
    ...
    def test_validation_catches_energy_errors(self):
        ...
        assert x.validate_energy_conservation() is True

        # Artificially introduce drift
        x.E_lattice += 1

        # Validation should detect and fix (returns False)
        assert x.validate_energy_conservation() is False
    ...
    def test_bond_count_validation_detects_errors(self):
        ...
        # Validation should pass initially
        assert x.validate_bond_counts() is True
        ...
        assert x.validate_bond_counts() is False
...
class TestValidationIntegration:
    ...
    def test_validation_catches_actual_corruption(self):
        ...
        assert (
            sim.E_lattice + sim.E_demon_sum == initial_total
        ...
        assert (
            result is True
        ), "Bond count validation should pass when no corruption exists"

    def test_validation_overhead_scales_with_mode(self):
        """Verify that different validation modes have different
        ...
        assert (
            time_off >= 0 and time_frequent >= 0
