...
class TestBondChangeOperation:
    ...
    def test_bond_change_bond_count_consistency(self):
        ...
        for _ in range(50):
            sim.bond_change(np.random.randint(0, sim.N), np.random.randint(0, sim.N))
            ...
            assert np.array_equal(sim.bond_count, actual_counts), (
                f"Bond count mismatch: tracked {sim.bond_count} vs "
...
class TestBondUpdateEdgeCases:
    ...
    def test_left_neighbor_bond_sign_flip(self):
        """Test that bond_change properly updates left neighbor bond when it has wrong
        sign.

        This specifically tests lines 222-226 in inferno.py where old_bond != new_bond.
        The bond_change function should detect and fix inconsistencies in the left
        neighbor bond.
        """
        for SimClass in [Inferno, irrInferno]:
            sim = SimClass(N=100, R=5, validate_mode="off")
            ...
            # Bond 49 (from 49 to 50): should be +1 (anti-aligned) - but we'll set it
            # wrong
            ...
            assert sim.bonds[49] == expected_bond_49, (
                f"{SimClass.__name__}: Bond 49 should be corrected to "
                f"{expected_bond_49}, got {sim.bonds[49]}"
            )

            # Verify counts are still consistent
            actual_counts = np.bincount(sim.bonds + 1, minlength=3).astype(np.int64)
            assert np.array_equal(sim.bond_count, actual_counts), (
                f"{SimClass.__name__}: Bond counts should match after left "
                "neighbor update"
            )
...
class TestIntegerArithmetic:
    ...
    def test_no_floating_point_drift(self):
        ...
        assert sim.E_total == initial_energy
        assert isinstance(sim.E_total, type(initial_energy))
